...
repeat
    ...
    local scan_result = redis.call("SCAN", cursor, "MATCH", "person:*:regions", "COUNT", 1000)
    cursor = scan_result[1]
    local keys = scan_result[2]

    for _, key in ipairs(keys) do
        local person_id = key:match('person:(%d+):regions')
        if person_id then
            local regions = redis.call('SMEMBERS', key)
